...
class Brain:
    ...
    def __init__(self, 
        ...
        # network
        self.main_q_network = freeze_ae(img_size, num_latent, num_actions, pretrained_model, variational)
        self.target_q_network = freeze_ae(img_size, num_latent, num_actions, pretrained_model, variational)
...
class Agent:
    ...
    def __init__(self,
        ...
        '''課題の状態と行動の数を設定する'''
        self.brain = Brain(img_size, num_actions, num_latent, mem_capacity, batch_size, lr, 
                            gamma, debug, device, pretrained_model, variational)  # エージェントが行動を決定するための頭脳を生成
